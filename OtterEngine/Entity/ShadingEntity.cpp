...
std::vector<std::unique_ptr<GraphicsResource>> ShadingEntity::s_shadingResources;

ShadingEntity::ShadingEntity(const Graphics& graphics, const Vector3& translation, const Vector3& rotation, const Vector3& scale,
	...
	if (s_shadingResources.empty()) {
		...
		if (kRenderMethod == RenderMethod::Gouraud) {
			...
			s_shadingResources.push_back(std::make_unique<PixelShader>(graphics, L"GouraudPixelShader.cso"));
			s_shadingResources.push_back(std::make_unique<InputLayout>(graphics, vertexShaderBlob, InputLayout::LayoutType::Shading));
		}
			...
			const std::vector<uint8_t> vertexShaderBlob = pVertexShader->GetVertexShaderBlob();
			s_shadingResources.push_back(std::move(pVertexShader));
			s_shadingResources.push_back(std::make_unique<PixelShader>(graphics, L"PhongPixelShader.cso"));
			s_shadingResources.push_back(std::make_unique<InputLayout>(graphics, vertexShaderBlob, InputLayout::LayoutType::Shading));
		}
	}
