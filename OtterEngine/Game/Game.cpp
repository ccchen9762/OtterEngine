...
Game::Game() :
    ...
    m_timer(Timer()),
    m_alive(true), 
    m_renderGraph(*(m_mainWindow.m_pGraphics)),
    m_mainCamera(*(m_mainWindow.m_pGraphics), Vector3(20.0f, 15.0f, 0.0f), Vector3(-1.0f, 0.0f, 0.0f), Vector3(0.0f, 1.0f, 0.0f)),
    ...
    m_pointLights.Register(m_renderGraph);
    for (const std::unique_ptr<Entity>& item : m_debugList) { item->Register(m_renderGraph); }
    for (const std::unique_ptr<Entity>& item : m_renderList) { item->Register(m_renderGraph); }
    //for (const std::unique_ptr<Model>& item : m_modelList) { item->Register(m_renderGraph); }
}
...
void Game::HandleInput(double deltaTime) {
    ...
    if (m_mainWindow.m_keyboard.IsKeyPressed(0x57)) {
        m_mainCamera.TranslateCameraZ(deltaTime * 5000.0f);
    }
    // S
    ...
    if (m_mainWindow.m_keyboard.IsKeyPressed(0x41)) {
        m_mainCamera.TranslateCamera(-deltaTime * 4000.0f, 0.0f);
    }
    // D
    ...
    if (m_mainWindow.m_keyboard.IsKeyPressed(VK_SPACE)) {
        // minus goes up
        m_mainCamera.TranslateCamera(0.0f, -deltaTime * 4000.0f);
    }
    // L Shift
    ...
    while (!m_mainWindow.m_mouse.MouseEventBufferEmpty()) {
        ...
        if (mouseEvent.getEventType() != Mouse::MouseEvent::Type::Empty) {
            ...
            case Mouse::MouseEvent::Type::WheelDown: {
                ...
                m_mainCamera.TranslateCameraZ(deltaTime * -100000.0f);
                break;
            }
            ...
            case Mouse::MouseEvent::Type::Move: {
                ...
                if (m_mainWindow.m_mouse.IsMButtonPressed()) {
                    m_mainCamera.TranslateCamera(translateX, translateY);
                }
                if (m_mainWindow.m_mouse.IsRButtonPressed()) {
                    m_mainCamera.RotateCamera(translateX, translateY);
                }
                prevPos = pos;
                break;
            }
...
void Game::Update(double deltaTime) {
    ...
    m_mainCamera.Update(*(m_mainWindow.m_pGraphics));
    ...
    for (const std::unique_ptr<Entity>& item : m_debugList) {
        item->Update();
        item->AssignJob(); 
    }

    for (const std::unique_ptr<Entity>& item : m_renderList) {
        item->Update();
        item->AssignJob();
    }

    /*for (const std::unique_ptr<Model>& item : m_modelList) {
        item->Update();
        item->AssignJobs();
    }*/

    m_renderGraph.Execute(*(m_mainWindow.m_pGraphics));
