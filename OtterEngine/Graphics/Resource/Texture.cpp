...
Texture::Texture(const Graphics& graphics, const std::wstring& path) {
...
void Texture::Bind(const Graphics& graphics) const {
	GetDeviceContext(graphics)->PSSetSamplers(0u, 1u, m_pSampler.GetAddressOf());
	GetDeviceContext(graphics)->PSSetShaderResources(0u, 1u, m_pTextureView.GetAddressOf());
}